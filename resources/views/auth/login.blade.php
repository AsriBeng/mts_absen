...
<body class="bg-gradient-to-br from-emerald-50 via-slate-50 to-emerald-100 flex items-center justify-center min-h-screen p-4">
    ...
    <div class="w-full max-w-md bg-white rounded-2xl shadow-xl border border-emerald-100 p-8">
        ...
        <div class="text-center mb-8">
            <div class="inline-block p-3 rounded-full bg-emerald-50 mb-3 border border-emerald-100 shadow-sm">
            ...
            <p class="text-sm font-medium text-emerald-600 mt-0.5">Sistem Presensi Digital</p>
        ...
        {{-- Pesan Status --}}
        @if (session('status'))
            <div class="mb-5 bg-emerald-50 border-l-4 border-emerald-500 text-emerald-700 p-4 rounded-r-lg shadow-sm text-sm">
                {{ session('status') }}
            </div>
        @endif
        ...
        <form action="{{ route('login') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <div class="relative">
                    <input type="email" name="email" id="email" value="{{ old('email') }}" required autofocus
                        ...
                        class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm text-slate-800 transition duration-150 outline-none">
                </div>
            </div>
            ...
            <div>
                <button type="submit"
                    class="w-full bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white font-semibold py-3 px-4 rounded-xl shadow-lg shadow-emerald-600/20 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 transition duration-200">
                    Masuk ke Sistem
                </button>
            </div>
        </form>
