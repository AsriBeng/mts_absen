    ...
    <header class="h-20 bg-emerald-900 text-white px-4 sm:px-6 flex items-center justify-between border-b border-emerald-950 shadow-md sticky top-0 z-30 shrink-0">
        ...
        <div class="relative" x-data="{ profileDropdownOpen: false }">
            ...
            <button @click="profileDropdownOpen = !profileDropdownOpen"
                    ...
                    class="flex items-center gap-3 bg-white/10 hover:bg-white/20 px-3 py-1.5 rounded-2xl border border-emerald-700/50 transition focus:outline-none">

                {{-- Role / Jabatan & Nama User --}}
                <div class="text-right hidden sm:block">
                    <p class="text-[11px] text-emerald-200 font-medium capitalize leading-none">
                        {{ Auth::user()->role->name ?? 'Guru' }}
                    </p>
                ...
                <img src="{{ asset('image/logo.png') }}"
                     alt="User Avatar"
                     class="w-9 h-9 rounded-xl border border-emerald-400 object-cover bg-white p-0.5 shadow-sm">
            </button>
            ...
            <div x-show="profileDropdownOpen"
                ...
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit" class="w-full flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-rose-600 hover:bg-rose-50 transition text-left">
