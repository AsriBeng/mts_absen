...
<div class="space-y-6" x-data="{ showBarcodeModal: false }">
    ...
    <div class="bg-emerald-900 text-white rounded-2xl p-6 shadow-md flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <button @click="showBarcodeModal = true"
                class="bg-emerald-500 hover:bg-emerald-600 text-white font-semibold text-sm px-5 py-3 rounded-xl shadow-lg shadow-emerald-950/30 flex items-center gap-2 transition shrink-0">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="p-5 border-b border-slate-100 flex items-center justify-between">
            ...
            <span class="text-xs bg-emerald-50 text-emerald-600 font-bold px-3 py-1 rounded-full border border-emerald-100">{{ date('d M Y') }}</span>
        </div>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($attendances as $item)
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-6 py-4">
                                <span class="px-2.5 py-1 rounded-full text-xs font-semibold {{ $item->status === 'hadir' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600' }}">
                                    {{ ucfirst($item->status) }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                @if($item->image_in)
                                    <button class="text-xs text-emerald-600 hover:underline font-semibold">Lihat Foto</button>
                                @else
            ...
            <div x-show="showBarcodeModal"
                ...
                <div class="bg-slate-50 p-4 rounded-2xl border border-slate-200 inline-block mb-3">
                    @php
                        ...
                        $qrUrl = "https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=" . urlencode($code);
                    @endphp
                    <img id="qrImage" src="{{ $qrUrl }}" alt="QR Code Absensi" class="w-48 h-48 mx-auto object-contain rounded-lg shadow-sm">
                </div>
                ...
                {{-- 2 Tombol Aksi: Generate & Download --}}
                <div class="grid grid-cols-2 gap-3">
                    {{-- Form Generate Baru --}}
                    <form action="{{ route('admin.absensi.generate') }}" method="POST">
                        @csrf
                        <button type="submit" class="w-full bg-amber-500 hover:bg-amber-600 text-white font-semibold text-xs py-3 px-3 rounded-xl transition flex items-center justify-center gap-2">
                            <i class="fas fa-sync-alt"></i> Generate
                        </button>
                    </form>

                    {{-- Tombol Download --}}
                    <a href="{{ $qrUrl }}" download="QR_Absensi_MA_AlHuda.png" target="_blank"
                       class="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-xs py-3 px-3 rounded-xl transition flex items-center justify-center gap-2">
